...
if (isset($_POST['upd_contacts'])) {
    ...
    $q = "UPDATE `contact_details` SET `address`=?,`gmap`=?,`pn1`=?,`pn2`=?,`email`=?,`fb`=?,`ins`=?,`tw`=?,`iframe`=? WHERE `sr_no`=?";
...
if (isset($_POST['add_member'])) {
    ...
    if (!isset($_FILES['member_picture'])) {
        echo 'no_file';
        exit;
    }
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        echo <<<data
                ...
                <div class="card bg-dark text-white position-relative">
                    ...
                    <p class="card-text mb-0">{$row['name']}</p>
                    </div>
                </div>
                </div>
                data;
