...
if (isset($_POST['add_room'])) {
    ...
    if ($flag == 1) {
        $room_id = mysqli_insert_id($conn); // Ensure this matches your db_config connection variable

        // Fixed: Added 'true' to decode as array for consistency
        $features = json_decode($_POST['features'], true);
        $facilities = json_decode($_POST['facilities'], true);
...
if (isset($_POST['get_room'])) {
    ...
    // Logic Fix: Create a 'desc' key so JavaScript finds it easily
    $roomdata['desc'] = $roomdata['description'];
...
if (isset($_POST['thumb_image'])) {
    ...
    $q = "UPDATE `room_images` SET `thumb`=? WHERE `room_id`=? AND `sr_no`=?";
    $v = [1, $frm_data['room_id'], $frm_data['image_id']];
    $res = update($q, $v, 'iii');
...
if (isset($_POST['remove_room'])) {
    ...
    // Cleanup relations (may return 0 – that’s OK)
    delete("DELETE FROM `room_images` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
    delete("DELETE FROM `room_features` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
    delete("DELETE FROM `room_facilities` WHERE `room_id`=?", [$frm_data['room_id']], 'i');

    // Soft delete room (THIS is the real success condition)
    $res = update(
        "UPDATE `rooms` SET `removed`=? WHERE `id`=?",
        [1, $frm_data['room_id']],
        'ii'
    );
